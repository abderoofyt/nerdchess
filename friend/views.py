...
def game_list_view(request, *args, **kwargs):
    ...
    if user.is_authenticated:
        ...
        if user_id:
            ...
            try:
                friend_list = GameList.objects.get(user=this_user)
            ...
            if user != this_user:
                if not user in friend_list.games.all():
                    return HttpResponse("You must be friends to view their friends list.")
            games = [] # [(friend1, True), (friend2, False), ...]
            # get the authenticated user's friend list
            auth_user_friend_list = GameList.objects.get(user=user)
            for friend in friend_list.games.all():
                games.append((friend, auth_user_friend_list.is_mutual_friend(friend)))
            context['friends'] = games
    else:		
        return HttpResponse("You must be friends to view their friends list.")
    return render(request, "friend/game_list.html", context)
