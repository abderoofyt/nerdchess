...
@login_required
def room(request, room_name, friend_id, room_turn):
...
@login_required
def game_choice(request, friend_id):
    friend = User.objects.filter(pk=friend_id)
    if not friend:
        messages.error(request, 'Invalid User ID')
        return redirect('game-enroll') 
    if not FriendList.objects.filter(user=request.user, friends=friend[0]):
        messages.error(request, 'You need to be Gaming to chat')
        return redirect('game-enroll') 
    ...
    if not room:
        ...
        create_room.save()
        room = create_room.room_id
        return redirect('game', room, friend_id)

    return redirect('game', room[0].room_id, friend_id)

""" Chatroom between users """
@login_required
def game(request, room_name, friend_id):
    all_rooms = Room.objects.filter(room_id=room_name)
    if not all_rooms:  
        messages.error(request, 'Invalid Room ID')
        return redirect('game-enroll')
